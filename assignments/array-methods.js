...
runners.forEach(function(element) {
    fullName.push(`First Name: ${element.first_name}, Last Name: ${element.last_name}`);
});
console.log(fullName);

// ==== Challenge 2: Use .map() ====
// The event director needs to have all the runner's first names converted to uppercase because the director BECAME DRUNK WITH POWER. Convert each first name into all caps and log the result
let allCaps = runners.map(function(charlestonChew) {
    return charlestonChew.first_name.toUpperCase();
})
...
let largeShirts = runners.filter((garfieldLasagna) => {
    return garfieldLasagna.shirt_size === "L";
});
console.log(largeShirts);

// ==== Challenge 4: Use .reduce() ====
// The donations need to be tallied up and reported for tax purposes. Add up all the donations into a ticketPriceTotal array and log the result
let ticketPriceTotal = runners.reduce((total, waffleHouse) => {
    return total + waffleHouse.donation;
}, 0);
...
// Problem 1
let allNotCaps = runners.map(function(cherryIce) {
    return cherryIce.first_name.toLowerCase();
})


console.log(allNotCaps); 
// Problem 2

let comDon = [];
runners.forEach(function(element) {
    comDon.push(`Company Name: ${element.company_name}, Donation: ${element.donation}`);
});
console.log(comDon);

// Problem 3
let largeDonation = runners.filter((garfieldLasagna) => {
    return garfieldLasagna.donation >= 100;
});
